"""
The :mod:`hspipy.utils.discovery` module includes utilities to discover
objects (i.e. estimators, displays, functions) from the `hspipy` package.
"""
...
def all_estimators(type_filter=None):
    """Get a list of all estimators from `hspipy`.
    ...
    all_classes = []
    root = str(Path(__file__).parent.parent)  # hspipy package
    # Ignore deprecation warnings triggered at import time and from walking
    # packages
    with ignore_warnings(category=FutureWarning):
        for _, module_name, _ in pkgutil.walk_packages(
            path=[root], prefix="hspipy."
        ):
...
def all_displays():
    """Get a list of all displays from `hspipy`.
...
def _is_checked_function(item):
    ...
    mod = item.__module__
    if not mod.startswith("hspipy.") or mod.endswith("estimator_checks"):
        return False
...
def all_functions():
    """Get a list of all functions from `hspipy`.
